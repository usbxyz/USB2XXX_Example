Module USB2XXX_IIC_24C128
    ...
    Sub Main()
        ...
        Dim WriteDataBuffer(2 * 1024 - 1) As Byte
        Dim PageSize As Int32 = 16  '芯片页大小，每次写数据最大数据量不能超过该大小
        Dim i As Integer
        Dim Addr As UInt16 = 0
        Dim SlaveAddr As UInt16 = &H50
        For i = 0 To (WriteDataBuffer.Length - 1)
            WriteDataBuffer(i) = CType(Math.Round(Rnd() * 255), Byte)    '这里用随机数填充数据缓冲区
        Next i
        '循环将数据写入芯片
        Dim WriteDataBufferTemp(PageSize + 1 - 1) As Byte
        System.Console.WriteLine("开始循环写数据...")
        timer.Start()
        For i = 0 To (WriteDataBuffer.Length / PageSize) - 1
            Addr = i * PageSize '写入数据起始地址
            WriteDataBufferTemp(0) = Addr And &HFF   '内部存储区地址低8位
            SlaveAddr = &H50 Or ((Addr >> 8) And &H7)   '内部存储区地址的高3位
            '循环将数据缓冲区中的数据拷贝到该数组中
            For j = 0 To (PageSize - 1)
                WriteDataBufferTemp(1 + j) = WriteDataBuffer(Addr + j)
            Next j
            '将数据写入芯片
            ret = IIC_WriteBytes(DeviceHandle(DevIndex), IICIndex, SlaveAddr, WriteDataBufferTemp, WriteDataBufferTemp.Length, 100)
            If ret <> IIC_SUCCESS Then
        ...
        timer.Restart()
        Dim ReadDataBuffer(2 * 1024 - 1) As Byte
        Addr = 0    '从0地址开始读数据
        WriteDataBufferTemp(0) = Addr And &HFF   '内部存储区地址低8位
        SlaveAddr = &H50 Or ((Addr >> 8) And &H7)   '内部存储区地址的高3位
        '该适配器每次I2C读取数据可以达20K字节，由于芯片容量小于20K，所以这里可以一次性读出所有数据，不过这样做这个函数耗时可能会比较多，也可以考虑用循环的方式读出数据
        ret = IIC_WriteReadBytes(DeviceHandle(DevIndex), IICIndex, SlaveAddr, WriteDataBufferTemp, 1, ReadDataBuffer, ReadDataBuffer.Length, 100)
        If ret <> IIC_SUCCESS Then
