...
int main(int argc, const char* argv[])
{
...
#endif
    //初始化配置I2C
    ret = SMBUS_Init(DevHandle[0],IICIndex,100000,1);
    if(ret != IIC_SUCCESS){
...
#endif
    //SMBUS_QuickCommand
    ret = SMBUS_QuickCommand(DevHandle[0],IICIndex,SlaveAddr,1);
    if(ret != IIC_SUCCESS){
    ...
    //SMBUS_WriteByte
    ret = SMBUS_WriteByte(DevHandle[0],IICIndex,SlaveAddr,0x00,1);
    if(ret != IIC_SUCCESS){
