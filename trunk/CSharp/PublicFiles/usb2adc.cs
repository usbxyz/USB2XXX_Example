...
namespace USB2XXX
{
    class USB2ADC
    {
        ...
        //定义连续采集数据模式下的回调函数
        public delegate Int32 ADC_GET_DATA_HANDLE(Int32 DevHandle, UInt16[] pData, Int32 DataNum);//接收数据回掉函数
        //USB2ADC相关函数定义
        /**
          * @brief  初始化配置ADC
        ...
        public static extern Int32 ADC_Init(Int32 DevHandle,Byte Channel,Int32 SampleRateHz);
        /**
          * @brief  启动并驱动ADC转换值
          * @param  DevHandle 设备句柄
          ...
          */
        [DllImport("USB2XXX.dll")]
        public static extern Int32 ADC_Read(Int32 DevHandle, UInt16[] pData, Int32 DataNum);
        /**
          * @brief  启动ADC连续转换模式
          * @param  DevHandle 设备句柄
          * @param  Channel 需要配置的ADC通道，每个bit为对应一个通道，为1时则需要配置该通道，最低位代表通道0
          ...
          */
        [DllImport("USB2XXX.dll")]
        public static extern Int32 ADC_StartContinueRead(Int32 DevHandle,Byte Channel,Int32 SampleRateHz,Int32 FrameSize,ADC_GET_DATA_HANDLE pGetDataHandle);
        /**
          * @brief  停止ADC连续转换模式
          * @param  DevHandle 设备句柄
          * @retval 函数执行状态，小于0函数执行出错
          */
        [DllImport("USB2XXX.dll")]
        public static extern Int32 ADC_StopContinueRead(Int32 DevHandle);
        /**
          ...
          */
        [DllImport("USB2XXX.dll")]
        public static extern Int32 ADC_GetData(Int32 DevHandle,UInt16[] pDataBuffer,Int32 BufferSize);
    }
}
