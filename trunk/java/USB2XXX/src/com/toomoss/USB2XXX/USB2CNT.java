...
public interface USB2CNT extends Library {
	...
	//定义函数返回错误代码
	int CNT_SUCCESS             =(0);   //函数执行成功
	int CNT_ERR_NOT_SUPPORT     =(-1);  //适配器不支持该函数
	int CNT_ERR_USB_WRITE_FAIL  =(-2);  //USB写数据失败
	int CNT_ERR_USB_READ_FAIL   =(-3);  //USB读数据失败
	int CNT_ERR_CMD_FAIL        =(-4);  //命令执行失败
	//计数器模式
	byte COUNTER_MODE_UP     =0;
	byte COUNTER_MODE_DOWN   =1;
	//计数器触发条件
	byte COUNTER_POL_RISING     =0;
	byte COUNTER_POL_FALLING    =1;
	byte COUNTER_POL_BOTHEDGE   =2;
	//计数器宽度
	byte COUNTER_BITS16        =16;
	byte COUNTER_BITS32        =32;
	//计数器输入引脚模式
	byte  COUNTER_PIN_NOPULL     =0x00;
	byte  COUNTER_PIN_UP         =0x01;
	byte  COUNTER_PIN_DOWN       =0x02;
	//计数器通道定义
	byte COUNTER_CH0             =0x01;
	byte COUNTER_CH1             =0x02;
	byte COUNTER_CH2             =0x04;
	byte COUNTER_CH3             =0x08;
