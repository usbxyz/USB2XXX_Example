...
public interface USB_Device extends Library{
	...
	/**
	  ...
	  * @retval 获取设备信息的状态
	  */
	boolean USB_GetDeviceInfo(int DevHandle,DEVICE_INFO pDevInfo,byte[] pFunctionStr);
	...
	/**
	  ...
	  * @retval 用户区数据擦出状态
	  */
	boolean USB_EraseUserData(int DevHandle);
	...
	/**
	  ...
	  * @retval 写入用户自定义数据状态
	  */
	boolean USB_WriteUserData(int DevHandle,int OffsetAddr,byte[] pWriteData,int DataLen);
	...
	/**
	  ...
	  * @retval 读出用户自定义数据的状态
	  */
	boolean USB_ReadUserData(int DevHandle,int OffsetAddr,byte[] pReadData,int DataLen);
}
