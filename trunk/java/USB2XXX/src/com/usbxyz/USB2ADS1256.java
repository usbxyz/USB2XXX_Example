...
public interface USB2ADS1256 extends Library {
	...
	/**
	  ...
	  */
	int  ADS1256_Init(int DevIndex,char Channel,ADS1256_CONFIG pConfig);
	/**
	  ...
	  * @retval 函数执行状态，小于0函数执行出错
	  */
	int  ADS1256_Start(int DevIndex,char Channel);
	/**
	  * @brief  启动ADC连续转换模式
	  * @param  DevIndex 设备索引号
	  ...
	  */
	int  ADS1256_Read(int DevIndex,char Channel,int[] pData);
	/**
	  ...
	  * @retval 函数执行状态，小于0函数执行出错
	  */
	int  ADS1256_Stop(int DevIndex,char Channel);
