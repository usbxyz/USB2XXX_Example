...
if __name__ == '__main__': 
    ...
    ret = IIC_GetSlaveAddr(DevIndex,IICIndex,byref(SlaveAddr),byref(SlaveAddrNum))
    if ret != IIC_SUCCESS:
    ...
    for i in range(0,8):
        WriteBuffer[1+i] = i
    ret = IIC_WriteBytes(DevIndex,IICIndex,0x50,byref(WriteBuffer),9,100)
    if ret != IIC_SUCCESS:
    ...
    for i in range(0,8):
        WriteBuffer[1+i] = i
    ret = IIC_WriteReadBytes(DevIndex,IICIndex,0x50,byref(WriteBuffer),1,byref(ReadBuffer),8,100)
    if ret != IIC_SUCCESS:
