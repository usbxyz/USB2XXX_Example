...
int main(int argc, const char* argv[])
{
    ...
    CANConfig.CAN_BS2 = 1;
    CANConfig.CAN_SJW = 1;
    ret = CAN_Init(DevIndex,CANIndex,&CANConfig);
    if(ret != CAN_SUCCESS){
    ...
    CAN_Filter_Init(DevIndex,CANIndex,&CANFilter);
    ...
    int SendedNum = CAN_SendMsg(DevIndex,CANIndex,CanMsg,5);
    if(SendedNum >= 0){
    ...
    ret = CAN_GetStatus(DevIndex,CANIndex,&CANStatus);
    if(ret == CAN_SUCCESS){
        printf("TSR = %08X\n",CANStatus.TSR);
        printf("ESR = %08X\n",CANStatus.ESR);
    }else{
    ...
    int CanNum = CAN_GetMsg(DevIndex,CANIndex,CanMsgBuffer);
    if(CanNum > 0){
        printf("CanNum = %d\n",CanNum);
