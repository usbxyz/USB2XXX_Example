...
typedef  struct  _CBL_CMD_LIST{
	unsigned char   EraseFlash;			//擦出APP储存扇区数据
	unsigned char	WriteBlockFlash;	//以多字节形式写数据
	unsigned char	ReadBlockFlash;		//以多字节形式读数据
	unsigned char   BlockWriteInfo;		//设置多字节写数据相关参数（写起始地址，数据量）
	unsigned char   BlockReadInfo;		//设置多字节数据读相关参数（读起始地址，数据量）
	unsigned char	OnlineCheck;		//检测节点是否在线，同时返回固件信息
	unsigned char	CmdSuccess;			//命令执行成功
	unsigned char	CmdFaild;			//命令执行失败
	unsigned char	SetBaudRate;		//设置节点波特率
	unsigned char	ExcuteApp;			//执行应用程序
	unsigned char	Undefine0;			//未定义，保留以后用
	unsigned char	Undefine1;			//未定义，保留以后用
	unsigned char	Undefine2;			//未定义，保留以后用
	unsigned char	Undefine3;			//未定义，保留以后用
...
int WINAPI CAN_BL_EraseFlash(int DevIndex,int CANIndex,unsigned short NodeAddr,unsigned int FlashStartAddr,unsigned int FlashEndAddr,unsigned int TimeOut);
int WINAPI CAN_BL_WriteBlock(int DevIndex,int CANIndex,unsigned short NodeAddr,unsigned int FlashStartAddr,unsigned char *pData,unsigned int DataNum,unsigned int TimeOut);
int WINAPI CAN_BL_ExcuteApp(int DevIndex,int CANIndex,unsigned short NodeAddr,unsigned int FlashStartAddr);
int WINAPI CAN_BL_SetBaudRate(int DevIndex,int CANIndex,unsigned short NodeAddr,PCAN_INIT_CONFIG pInitConfig,unsigned int BaudRate,unsigned int TimeOut);
