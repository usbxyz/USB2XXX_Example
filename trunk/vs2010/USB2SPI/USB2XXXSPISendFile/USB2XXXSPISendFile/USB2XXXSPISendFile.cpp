...
int main(int argc, const char* argv[])
{
    ...
    for(PackIndex = 0;PackIndex<(InputFileLen/PackSize);PackIndex++){
        ret = SPI_WriteBytes(DevIndex,SPIIndex,&pInputBuf[PackIndex*PackSize],PackSize);
        if(ret != SPI_SUCCESS){
    ...
    if(InputFileLen%PackSize){
        ret = SPI_WriteBytes(DevIndex,SPIIndex,&pInputBuf[PackIndex*PackSize],InputFileLen%PackSize);
        if(ret != SPI_SUCCESS){
            printf("SPI write data error!\n");
            return 0;
        }
