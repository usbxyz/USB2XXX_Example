...
int main(int argc, const char* argv[])
{
    ...
    ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteDataTemp,1,ReadDataTemp,3,0);
    if(ret != SPI_SUCCESS){
    ...
    //ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteDataTemp,1);


    WriteDataTemp[0] = 0x05;// Read Status Register-1
	ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteDataTemp,1,ReadDataTemp,1,100);
    printf("STATUS1 = %02X \n",ReadDataTemp[0]);
   ...
   /* return 0;
	...
	//ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteDataTemp,3);
    Sleep(1000);
    ...
    ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteDataTemp,4);
    if(ret != SPI_SUCCESS){
    ...
    do{
	    WriteDataTemp[0] = 0x05;// Read Status Register-1
	    ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteDataTemp,1,ReadDataTemp,1,0);
        Sleep(500);
    ...
    ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteDataTemp,256+4);
    if(ret != SPI_SUCCESS){
    ...
    WriteDataTemp[2] = 0x01;
    WriteDataTemp[3] = 0x00;
    ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteDataTemp,4,ReadDataTemp,256,0);
    if(ret != SPI_SUCCESS){
    ...
    for(addr = 0;addr < 0x800000;addr += 20480){
        ...
        ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteDataTemp,5,ReadDataTemp,20480,0);
        if(ret != SPI_SUCCESS){
            printf("Read Data error :%d\n",ret);
            break;
        }
    ...
    ret = SPI_FlashInit(DevIndex,SPIIndex,50000000>>2,&SPIFlashConfig);
    if(ret != SPI_SUCCESS){
        printf("Initialize Device Error!\n");
        return 0;
    }
    ...
    ret = SPI_FlashEraseSector(DevIndex,SPIIndex,0,1);
    if(ret != SPI_SUCCESS){
    ...
    ret = SPI_FlashWrite(DevIndex,SPIIndex,10,TestBuffer,200);
    if(ret != SPI_SUCCESS){
        printf("Flash Write Error!\n");
        return 0;
    }
    ret = SPI_FlashReadFast(DevIndex,SPIIndex,0,TestBuffer,256);
    if(ret != SPI_SUCCESS){
    ...
    ret = SPI_FlashWrite(DevIndex,SPIIndex,20,TestBuffer,256);
    if(ret != SPI_SUCCESS){
        printf("Flash Write Error!\n");
        return 0;
    }
    ret = SPI_FlashReadFast(DevIndex,SPIIndex,0,TestBuffer,512);
    if(ret != SPI_SUCCESS){
        printf("Flash Read Error!\n");
        return 0;
    }
   ...
   ret = SPI_FlashEraseSector(DevIndex,SPIIndex,0,1);
    ...
    ret = SPI_FlashWrite(DevIndex,SPIIndex,250,TestBuffer,270);
    if(ret != SPI_SUCCESS){
        printf("Flash Write Error!\n");
        return 0;
    }
    ret = SPI_FlashReadFast(DevIndex,SPIIndex,0,TestBuffer,256*3);
    if(ret != SPI_SUCCESS){
        printf("Flash Read Error!\n");
        return 0;
    }
    ...
    /*for(int i=0;i<((SPIFlashConfig.NumPages*SPIFlashConfig.PageSize)/SPIFlashConfig.SectorSize);i++){
        ret = SPI_FlashEraseSector(DevIndex,SPIIndex,i,1);
        if(ret != SPI_SUCCESS){
            printf("Erase Sector Error!\n");
            return 0;
        }
    ...
    //擦出整片数据，该函数擦出速度最快
    SPI_FlashEraseChip(DevIndex,SPIIndex);
    //计算消耗的时间并算出速度
    QueryPerformanceCounter(&litmp);// Get the current value of the performance counter
    EndTime = litmp.QuadPart; // Stop time
    // Print the write data speed information
    ...
    for(int i=0;i<((SPIFlashConfig.NumPages*SPIFlashConfig.PageSize)/sizeof(WriteBuffer));i++){
        ret = SPI_FlashWrite(DevIndex,SPIIndex,i*sizeof(WriteBuffer),WriteBuffer,sizeof(WriteBuffer));
        if(ret != SPI_SUCCESS){
            printf("Flash Write Error!\n");
            return 0;
        }
    ...
    for(int i=0;i<((SPIFlashConfig.NumPages*SPIFlashConfig.PageSize)/sizeof(ReadBuffer));i++){
        ret = SPI_FlashReadFast(DevIndex,SPIIndex,i*sizeof(ReadBuffer),ReadBuffer,sizeof(ReadBuffer));
        if(ret != SPI_SUCCESS){
            printf("Flash Read Error!\n");
            return 0;
        }
