...
int main(int argc, const char* argv[])
{
    ...
    for(int i=0,addr=0;i<DATA_SIZE;addr++){
        //发送WREN命令
        WriteBuffer[0]=0x06;
        ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteBuffer,1);
        if(ret != SPI_SUCCESS){
        ...
        ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteBuffer,4);
        if(ret != SPI_SUCCESS){
            printf("Write byte error!\n");
            return -1;
        }
        ...
        do{
            ...
            ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteBuffer,1,ReadDataBuffer,1,0);
            if(ret != SPI_SUCCESS){
    ...
    for(int i=0,addr=0;i<DATA_SIZE;addr++){
        ...
        WriteBuffer[k++] = addr>>8;//地址高8位
        WriteBuffer[k++] = addr&0xFF;//地址低8位
        ret = SPI_WriteReadBytes(DevIndex,SPIIndex,WriteBuffer,3,ReadDataBuffer,1,0);
        if(ret != SPI_SUCCESS){
            printf("WriteRead byte error!\n");
            return -1;
        }else{
