...
int main(int argc, const char* argv[])
{
    ...
    {
        //唤醒传感器
        IIC_WriteBytes(DevIndex,IICIndex,0x5C,NULL,0,10);
        Sleep(10);
        //发送数据测量命令
        unsigned char WriteBuffer[]={0x03,0x00,0x04};
        ret = IIC_WriteBytes(DevIndex,IICIndex,0x5C,WriteBuffer,sizeof(WriteBuffer),10);
        if(ret != IIC_SUCCESS){
        ...
        ret = IIC_ReadBytes(DevIndex,IICIndex,0x5C,ReadBuffer,sizeof(ReadBuffer),10);
        if (ret != IIC_SUCCESS){
