...
int main(int argc, const char* argv[])
{
    ...
    for(int i=0;i<8;i++){
        ...
        ret = IIC_Init(DevIndex,i,&IIC_Config);
        if(ret != IIC_SUCCESS){
        ...
        WriteBuffer[0] = 0x01;
        WriteBuffer[1] = 0x00;
        ret = IIC_WriteBytes(DevIndex,i,0x48,WriteBuffer,2,10);
        if(ret == IIC_SUCCESS){
    ...
    while(true)
    {
        ...
        for(int i=0;i<IICIndex;i++){
            ...
            WriteBuffer[0] = 0x00;
            ret = IIC_WriteReadBytes(DevIndex,IICChannel[i],0x48,WriteBuffer,1,ReadBuffer,2,10);
            if (ret != IIC_SUCCESS){
