...
int main(int argc, const char* argv[])
{
    ...
    //指定引脚为低电平的时候发送数据
    ret = SPI_WriteBytesOfEvent(DevHandle[0],SPIIndex,WriteBuffer,sizeof(WriteBuffer),SPI_EVENT_P15,EVENT_TYPE_LOW,10000);
    if(ret != SPI_SUCCESS){
    ...
    getchar();
    //指定引脚为高电平的时候读取数据
    ret = SPI_ReadBytesOfEvent(DevHandle[0],SPIIndex,ReadBuffer,sizeof(ReadBuffer),SPI_EVENT_P15,EVENT_TYPE_HIGH,10000);
    if(ret != SPI_SUCCESS){
    ...
    ret = SPI_WriteReadBytesOfEvent(DevHandle[0],SPIIndex,WriteBuffer,sizeof(WriteBuffer),ReadBuffer,sizeof(ReadBuffer),IntervalTime,SPI_EVENT_P15,EVENT_TYPE_FALLING,10000);
    if(ret != SPI_SUCCESS){
