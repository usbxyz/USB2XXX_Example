...
int main(int argc, const char* argv[])
{
    ...
    //擦出用户数据区域的所有数据
    state = USB_EraseUserData(DevIndex);
    if(!state){
    ...
    state = USB_WriteUserData(DevIndex,0,UserData,sizeof(UserData));
    if(!state){
    ...
    //从用户区读取数据
    state = USB_ReadUserData(DevIndex,0,UserData,sizeof(UserData));
    if(!state){
        printf("User data write data error!\n");
        return 0;
    }else{
    ...
    for(int i=0;i<64;i++){
        ret = SPI_WriteBytesAsync(DevIndex,SPIIndex,WriteBuffer,sizeof(WriteBuffer));
        if(ret != SPI_SUCCESS){
