...
int main(int argc, const char* argv[])
{
    ...
    ret = GPIO_SetOutput(DevHandle[0], ResetPinMask, 1);
    if (ret != GPIO_SUCCESS)
    {
    ...
    ret = SPI_Init(DevHandle[0],ADS1256Index,&SPIConfig);
    if(ret != SPI_SUCCESS){
        printf("Initialize device error!\r\n");
        return 0;
    }
    //RESET引脚输出低脉冲复位ADS1256
    GPIO_Write(DevHandle[0], ResetPinMask, 0); //输出低电平
    Sleep(10);
    GPIO_Write(DevHandle[0], ResetPinMask, ResetPinMask);  //输出高电平
    //准备配置寄存器数据
    ...
    WriteBuffer[6] = 0xFF;
    ret = SPI_WriteBytesOfEvent(DevHandle[0], ADS1256Index, WriteBuffer, 7, DrdyPinMask, 0x00, 1000);
    if (ret != SPI_SUCCESS)
    {
    ...
    WriteBuffer[1] = 4;
    ret = SPI_WriteReadBytesOfEvent(DevHandle[0], ADS1256Index, WriteBuffer, 2, ReadBuffer,5,10,DrdyPinMask, 0x10, 1000);
    if (ret != SPI_SUCCESS)
    {
    ...
    ret = SPI_WriteReadBytesOfEvent(DevHandle[0], ADS1256Index, WriteBuffer, 3, ReadBuffer, 3, 10, DrdyPinMask, 0x10, 1000);
    if (ret != SPI_SUCCESS)
    {
        printf("SPI Event Write&Read Data Error!\r\n");
        return 0;
    ...
    //检测DRADY引脚下降沿之后读回数据
    int ReadDataNum = 512;
    ret = SPI_BlockReadBytesOfEvent(DevHandle[0], ADS1256Index, ReadBuffer, 3, ReadDataNum, DrdyPinMask, 0x10, 100);
    if (ret != SPI_SUCCESS)
    {
    ...
    ret = SPI_WriteBytesOfEvent(DevHandle[0], ADS1256Index, WriteBuffer, 1, DrdyPinMask, 0x10, 1000);
    if (ret != SPI_SUCCESS)
    {
        printf("SPI Event Write Data Error!\r\n");
        return 0;
    ...
    for (int i = 0; i < ReadDataNum; i++)
    {
        ...
        {
            WriteBuffer[2] = ((Channel << 4) | 0x08);
            ret = SPI_WriteReadBytesOfEvent(DevHandle[0], ADS1256Index, WriteBuffer, 6, ReadBuffer, 3, 10, DrdyPinMask, 0x10, 1000);
            if (ret != SPI_SUCCESS)
            {
                printf("SPI Event Write&Read Data Error!\r\n");
                return 0;
