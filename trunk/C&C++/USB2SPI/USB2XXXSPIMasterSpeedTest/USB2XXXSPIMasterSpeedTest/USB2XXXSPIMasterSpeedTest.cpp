...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    for(int i=0;i<PackNum;i++){
        ret = SPI_WriteBytes(DevIndex,SPIIndex,WriteBuffer,PackSize);
        if(ret != SPI_SUCCESS){
    ...
    StartTime = litmp.QuadPart;       // Start time
    for(int i=0;i<PackNum;i++){
        ret = SPI_WriteBytesAsync(DevIndex,SPIIndex,WriteBuffer,PackSize);
        if(ret != SPI_SUCCESS){
            printf("SPI write data error!\n");
            getchar();
            return 0;
    ...
    StartTime = litmp.QuadPart;       // Start time
    for(int i=0;i<PackNum;i++){
        ret = SPI_ReadBytes(DevIndex,SPIIndex,ReadBuffer,PackSize);
        if(ret != SPI_SUCCESS){
