...
public interface USB2OW extends Library  {
    ...
    //定义函数返回错误代码
    int OW_SUCCESS             =(0);   //函数执行成功
    int OW_ERR_NOT_SUPPORT     =(-1);  //适配器不支持该函数
    int OW_ERR_USB_WRITE_FAIL  =(-2);  //USB写数据失败
    int OW_ERR_USB_READ_FAIL   =(-3);  //USB读数据失败
    int OW_ERR_CMD_FAIL        =(-4);  //命令执行失败
    int OW_ERR_NO_ACK          =(-5);  //无从设备应答
