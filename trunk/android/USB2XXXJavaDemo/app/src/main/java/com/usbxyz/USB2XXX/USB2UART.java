...
public interface USB2UART extends Library {
	...
	//定义函数返回错误代码
	int UART_SUCCESS             =(0);   //函数执行成功
	int UART_ERR_NOT_SUPPORT     =(-1);  //适配器不支持该函数
	int UART_ERR_USB_WRITE_FAIL  =(-2);  //USB写数据失败
	int UART_ERR_USB_READ_FAIL   =(-3);  //USB读数据失败
	int UART_ERR_CMD_FAIL        =(-4);  //命令执行失败
	//数据位宽
	byte UART_WORD_LENGTH_8BIT     =0;
	byte UART_WORD_LENGTH_9BIT     =1;
	//停止位
	byte UART_STOP_BITS_1          =0;
	byte UART_STOP_BITS_05         =1;
	byte UART_STOP_BITS_2          =2;
	byte UART_STOP_BITS_15         =3;
	//奇偶校验位
	byte UART_PARITY_NO            =0;
	byte UART_PARITY_EVEN          =4;
	byte UART_PARITY_ODD           =6;
	//TE控制信号输出
	byte UART_TE_DISEN             =0x00;
	byte UART_TE_EN_LOW            =(byte)0x80;
	byte UART_TE_EN_HIGH           =(byte)0x81;
