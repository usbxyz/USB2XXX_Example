...
public interface USB2PWM extends Library {
	...
	//定义函数返回错误代码
	int PWM_SUCCESS             =(0);   //函数执行成功
	int PWM_ERR_NOT_SUPPORT     =(-1);  //适配器不支持该函数
	int PWM_ERR_USB_WRITE_FAIL  =(-2);  //USB写数据失败
	int PWM_ERR_USB_READ_FAIL   =(-3);  //USB读数据失败
	int PWM_ERR_CMD_FAIL        =(-4);  //命令执行失败
	...
	public class PWM_CONFIG  extends Structure{
		...
		short[] Precision = new short[8];  //占空比调节精度
