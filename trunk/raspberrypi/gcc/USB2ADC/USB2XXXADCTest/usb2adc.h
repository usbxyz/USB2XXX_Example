...
//定义连续采集数据模式下的回调函数
typedef  int (WINAPI ADC_GET_DATA_HANDLE)(int DevIndex,unsigned short *pData,int DataNum);//接收数据回掉函数
...
/**
  ...
  * @retval 函数执行状态，小于0函数执行出错
  */
int WINAPI ADC_StartContinueRead(int DevIndex,char Channel,int SampleRateHz,int FrameSize,ADC_GET_DATA_HANDLE *pGetDataHandle);
/**
  * @brief  停止ADC连续转换模式
  * @param  DevIndex 设备索引号
  * @retval 函数执行状态，小于0函数执行出错
