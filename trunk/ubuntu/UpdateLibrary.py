...
libraryFile = ''
headerFilePath = '/home/wdluo/USB2XXX/trunk/USB2XXX/USB2XXX/source/usb2xxx'
libusb1FilePath = '/usr/local/lib/libusb-1.0.so'
usbDeviceHeaderFile = '/home/wdluo/USB2XXX/trunk/USB2XXX/USB2XXX/source'
...
def copyLibFiles():
    for root, dirs, files in os.walk(os.getcwd(), topdown=False):
        for name in dirs:# Copy USB2XXX.dll and USB2XXX.lib
            if 'x86_64' in name and "64bit" in platform.architecture() :
                shutil.copy(libraryFile,os.path.join(root, name))
                shutil.copy(libusb1FilePath,os.path.join(root, name))
            if 'x86_32' in name and "32bit" in platform.architecture() :
                shutil.copy(libraryFile,os.path.join(root, name))
                shutil.copy(libusb1FilePath,os.path.join(root, name))
